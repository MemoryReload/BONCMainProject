...
static void dyld_callback(const struct mach_header *mhp, intptr_t vmaddr_slide)
{
    ...
    for (NSString *modName in mods) {
        ...
        if (modName) {
            ...
            if (cls) {
                [[BONCModuleManager sharedManager] registerDynamicModule:cls];
            }
        }
    }
    ...
    for (NSString *map in services) {
        ...
        if (!error) {
            if ([json isKindOfClass:[NSDictionary class]] && [json allKeys].count) {
                ...
                if (protocol && clsName) {
                    [[BONCServiceManager sharedManager] registerService:NSProtocolFromString(protocol) implClass:NSClassFromString(clsName)];
                }
